...
public class MainGame {
    ...
    public void newGame() {
        if (grid == null) {
            grid = new Grid(GlobalApp.numSquaresX, GlobalApp.numSquaresY);
        } else {
            prepareUndoState();
            saveUndoState();
            grid.clearGrid();
        }
        aGrid = new AnimationGrid(GlobalApp.numSquaresX, GlobalApp.numSquaresY);
            ...
            for (int yy : traversalsY) {
                ...
                if (tile != null) {
                    ...
                    if (next != null && next.getValue() == tile.getValue() && next.getMergedFrom() == null) {
                        ...
                        // The mighty haikhongbontam tile
    ...
    private List<Integer> buildTraversalsY(Cell vector) {
        ...
        for (int xx = 0; xx < GlobalApp.numSquaresY; xx++) {
            traversals.add(xx);
        }
        if (vector.getY() == 1) {
            Collections.reverse(traversals);
